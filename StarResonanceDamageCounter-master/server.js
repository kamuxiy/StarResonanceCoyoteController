...
async function main() {
    ...
    server.listen(server_port, () => {
        ...
        // 支持通过环境变量禁用浏览器自动打开（用于无头模式集成）
        const noOpen = process.env.NO_OPEN || process.env.SRDC_NO_OPEN || process.env.DISABLE_OPEN;
        if (noOpen && noOpen !== '0' && noOpen !== 'false') {
            logger.info('Browser auto-open disabled (NO_OPEN is set)');
        } else {
            let command;
